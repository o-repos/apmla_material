...
from hdhp.utils import copy_dict, weighted_choice
...
class Particle(object):
    ...
    def update(self, event):
        ...
        b_n, z_n, opened_table, log_likelihood_dn = self.sample_table(t_n, d_n, u_n)
    ...
    def update_time_kernel(self, t_n, z_n):
        ...
        self.time_kernel_prior[z_n] = self.alpha_0[0] + event_count - self.dish_counters[z_n], self.alpha_0[1] + (
            sum_integrals)
...
def _infer_single_thread(history, params):
    ...
    epsilon = 1e-10
    particles = initialize_particles(params, prng)
    ...
    for i, h_i in enumerate(history):
        ...
        if u_i not in time_history_per_user:
            initialize_user_history(u_i, time_history_per_user, doc_history_per_user, question_history_per_user)

        add_event_user_history(h_i, time_history_per_user, doc_history_per_user, question_history_per_user)
        ...
        if i > 0 and i % params.resample_every == 0:
            ...
            if params.num_particles > 1 \
                ...
                count_resamples += 1
                new_particles, new_table_history_with_user, new_dish_on_table_per_user = resample_particles(particles,
                                                                                                            normalized,
                                                                                                            prng,
                                                                                                            table_history_with_user,
                                                                                                            dish_on_table_per_user,
    ...
    final_particle = get_final_particle(particles, max_logweight, epsilon, params, weights, total, prng,
                                        time_history_per_user, doc_history_per_user,
                                        question_history_per_user, table_history_with_user, dish_on_table_per_user)
    # pool.close()
    with open(params.progress_file, mode='a') as temp:
...
def initialize_particles(params, prng):
...
def initialize_user_history(u_i, time_history_per_user, doc_history_per_user, question_history_per_user):
...
def add_event_user_history(h_i, time_history_per_user, doc_history_per_user, question_history_per_user):
    t_i, d_i, u_i, q_i = h_i
...
def resample_particles(particles, normalized, prng, table_history_with_user, dish_on_table_per_user, inferred_tables):
    ...
    for index in new_particle_indices:
        ...
        inferred_tables[new_particle.uid] = copy(inferred_tables[particles[index].uid])

    return new_particles, new_table_history_with_user, new_dish_on_table_per_user


def get_final_particle(particles, max_logweight, epsilon, params, weights, total, prng, time_history_per_user,
                       doc_history_per_user,
                       question_history_per_user, table_history_with_user, dish_on_table_per_user):
    for p_i in particles:
        if max_logweight is None or max_logweight < p_i.logweight:
            max_logweight = p_i.logweight
